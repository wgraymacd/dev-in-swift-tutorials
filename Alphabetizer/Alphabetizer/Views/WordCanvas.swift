...
struct WordCanvas: View {
    @Environment(Alphabetizer.self) private var alphabetizer // use @Environment to retrieve an Alphabetizer instance from the environment
    ...
    var body: some View {
        ...
        .onChange(of: alphabetizer.message) { oldValue, newValue in
            switch (oldValue, newValue) { // switch works for multiple variables
